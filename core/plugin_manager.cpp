...
void plugin_manager::start_plugins() {
    for (auto &plugin : loaded_plugins) {
        logging::log("[PluginManager::StartAllPlugins] Running HiOctaneEntry for plugin: {}...", plugin.name.c_str());
        plugin.start();
        logging::log("[PluginManager::StartAllPlugins] Finished running HiOctaneEntry for plugin: {}.", plugin.name);
    }
}

void plugin_manager::exit_plugins() {
    for (auto &plugin : loaded_plugins) {
        logging::log("[PluginManager::ExitAllPlugins] Running HiOctaneExit for plugin: {}...", plugin.name);
        plugin.exit();
        logging::log("[PluginManager::ExitAllPlugins] Finished running HiOctaneExit for plugin: {}.", plugin.name);

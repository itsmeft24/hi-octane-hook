...
int main(int argc, char *argv[]) {
  ...
  // Here, we load our DLL with DONT_RESOLVE_DLL_REFERENCES so we can grab the
  ...
  // check if we need to update and let the user know they are running an
  // outdated version
  ...
  if (argc > 1)
    CreateProcessA("cars_mater.exe", "cars_mater.exe -windowed", nullptr,
                   nullptr, TRUE, CREATE_SUSPENDED, nullptr, nullptr, &sInfoA,
                   &pInfo);
  else
    CreateProcessA("cars_mater.exe", "cars_mater.exe", nullptr, nullptr, TRUE,
                   CREATE_SUSPENDED, nullptr, nullptr, &sInfoA, &pInfo);
  ...
  HANDLE dll_loader_thread = CreateRemoteThread(
      pInfo.hProcess, 0, 0, (LPTHREAD_START_ROUTINE)LoadLibraryA, allocated_str,
      0, 0);

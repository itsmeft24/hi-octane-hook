...
#include "core/game/gfx/x360_video_card.hpp"
#include "hdr.hpp"
...
namespace HDRPatch {
	...
	DeclareFunction(X360FilterAlgorithm*, __thiscall, X360FilterAlgorithm_Initialize, 0x0040ce50, X360FilterAlgorithm*, char*, ParameterBlock*);
    ...
    DeclareFunction(void, __thiscall, X360VertexShaderManager_SetVertexShader, 0x004110a0, void*, void*, int);
    DeclareFunction(void, __thiscall, X360PixelShaderManager_SetPixelShader, 0x00411530, void*, void*);

	DeclareFunction(uintptr_t, __thiscall, X360FilterManager_GetBuffer, 0x0040de30, uintptr_t, int);

	DeclareFunction(void, __thiscall, X360FilterTexture_SetTexture, 0x0040f5b0, uintptr_t, int);
	DeclareFunction(void, __thiscall, X360FilterManager_ResolveFrameBuffer, 0x0040de00, uintptr_t, uintptr_t, int);
	...
	inline X360VertexShader* InitVertexShader(const std::string& shader_label) {
		X360VertexShader* memory = new X360VertexShader();
		memory->Create(shader_label.c_str(), 0);
		return memory;
	}

	inline X360PixelShader* InitPixelShader(const std::string& shader_label) {
		X360PixelShader* memory = new X360PixelShader();
		memory->Create(shader_label.c_str(), 0);
		return memory;
	}

	inline void SetShaders(X360VertexShader* vertex, X360PixelShader* pixel) {
		uintptr_t g_RenderTarget = RenderTarget();
		X360VertexShaderManager_SetVertexShader(*reinterpret_cast<void**>(g_RenderTarget + 0xC4), vertex, 0);
		X360PixelShaderManager_SetPixelShader(*reinterpret_cast<void**>(g_RenderTarget + 200), pixel);
	}
	...
	DefineReplacementHook(X360FilterStreamReadFromFileHook) {
		static void __fastcall callback(void* filter_stream, uintptr_t edx, ParameterBlock* pBlock, uintptr_t param_2)
		{
			X360FilterAlgorithm* filter_algorithm = nullptr;
			void* memory = nullptr;
			char algorithm_label[0x40]{};

			pBlock->GetParameter("FilterAlgorithm", "", algorithm_label, 0x40);
			...
			if (_stricmp(algorithm_label, "Null") == 0 || !X360VideoCard_IsFXEnabled(*g_VideoCard)) {
	...
	unsigned int __fastcall X360FilterHighDynamicRange::SendData(X360FilterHighDynamicRange* _this) {
		...
		// Idk what this is
		uintptr_t g_RenderTarget = RenderTarget();
		uintptr_t lpGlobalFilterManager = *reinterpret_cast<uintptr_t*>(0x006fd2b8);
		X360FilterManager_ResolveFrameBuffer(lpGlobalFilterManager, g_RenderTarget + 0x98, 4);
			
		// Set shaders.
		SetShaders(_this->DownsampleVert, _this->DownsamplePixel);
			
		// Set shader constants.
		float OnePixelOffset[] = { 1.0f / window_width, 1.0f / window_height, 0, 0 };
		d3d9()->SetPixelShaderConstantF(PixelShaderConstant::PS_HDR_OnePixelOffset, OnePixelOffset, 1);
			
		float MaxTexCoord[] = { 1.0f, 1.0f, 0, 0 };
		d3d9()->SetPixelShaderConstantF(PixelShaderConstant::PS_HDR_MaxTexCoord, MaxTexCoord, 1);
			
		// Set texture slots and render.
		uintptr_t this_00 = X360FilterManager_GetBuffer(lpGlobalFilterManager, 4);
		for (int x = 0; x < 15; x++) {
			X360FilterTexture_SetTexture(this_00, x);
		}
		/*
		...
		*/
		...
		for (int i = 0; i < kernelSize; i++) {
			blurOffsets[i] = (((float)i / (float)kernelSize) - 0.5f) * 0.008f;
		}
		...
		for (int i = 0; i < kernelSize; i++) {
			...
			float offset = (((float)i / (float)kernelSize) - 0.5f);
			kernelWeights[i] = exp(-(offset * offset) / (2 * sigma * sigma)) / static_cast<float>(sqrt(2.0 * std::numbers::pi * sigma * sigma));
			sum += kernelWeights[i];
		}
		...
		// Final compositing
		float UVOffset[] = { 1.0f / window_width, 1.0f / window_height, 1, 1 };
	...
	void __fastcall X360FilterHighDynamicRange::Initialize(X360FilterHighDynamicRange* _this, uintptr_t edx, char* name, ParameterBlock* pBlock)
	{
		X360FilterAlgorithm_Initialize(&_this->base, name, pBlock);
		pBlock->GetParameter("RotateBlur", _this->RotateBlur, &_this->RotateBlur);
		pBlock->GetParameter("NumBlurPasses", _this->NumBlurPasses, &_this->NumBlurPasses);
		pBlock->GetParameter("Threshold", _this->Threshold, &_this->Threshold);
		pBlock->GetParameter("PositiveRate", _this->PositiveRate, &_this->PositiveRate);
		pBlock->GetParameter("NegativeRate", _this->NegativeRate, &_this->NegativeRate);
		pBlock->GetParameter("FinalMixHDR", _this->FinalMixHDR, &_this->FinalMixHDR);
		pBlock->GetParameter("FinalMixLDR", _this->FinalMixLDR, &_this->FinalMixLDR);
		pBlock->GetParameter("Scale", _this->Scale, &_this->Scale);

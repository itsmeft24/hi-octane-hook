...
inline void* handle_callback(fs::CallbackInfo& info,
	...
	fs::CallbackContext clbkcontext{ base_filepath.c_str(), data,
	...
	fs::CarsFileCallback clbk =
		(fs::CarsFileCallback)InstalledCallbacks.at(base_filepath)
...
HIOCTANE_API void __stdcall HiOctane_RegisterFileCallback(
	...
	fs::CarsFileCallback callbackfunc) {
	InstalledCallbacks.insert_or_assign(
		filepath, fs::CallbackInfo{ max_size, callbackfunc });
}
...
DWORD* __stdcall BinkOpenHook(char* file, DWORD flags) {
	...
	utils::make_lowercase(base_filepath);                      // force lowercase

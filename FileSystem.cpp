...
inline void *HandleCallback(FileSystem::CallbackInfo &info,
  ...
  if (clbk(&clbkcontext) != 0) {
    ...
    auto filebackingpath =
        GenerateFileBackingPath(); // generate random string for filebacking
    ...
    f.write((char *)data,
            clbkcontext
                .FileSize); // size SHOULD be modified by the callback function
          ...
          if (entry.is_regular_file()) {
            ...
            if (fname == "mark_as_deleted.txt") {
              ...
              while (std::getline(file, line)) {
                ...
                if (std::find(MARK_AS_DELETED.begin(), MARK_AS_DELETED.end(),
                  ...
                  Logging::Log("[FileSystem::FileDiscovery] Added %s to the "
                               "deletion queue.\n",
                               line.c_str());
                }
              }
            ...
            if (entry.path().extension().string() == ".wav" ||
              ...
              if (MAP.find(base_filename) != MAP.end()) {
                Logging::Log("[FileSystem::FileDiscovery] File: %s in mod: %s "
                             "overwrites existing mod file in: %s\n",
                             base_filename.c_str(),
                             mod.path().filename().string().c_str(),
                             GetModForFile(base_filename).c_str());
                MAP.insert_or_assign(base_filename, entry.path());
              } else {
                Logging::Log(
                    "[FileSystem::FileDiscovery] Found file: %s in mod: %s\n",
                    base_filename.c_str(),
                    mod.path().filename().string().c_str());
                MAP.insert_or_assign(base_filename, entry.path());
              }
...
DWORD *__stdcall BinkOpenHook(char *file, DWORD flags) {
  ...
  if (_strnicmp(file, DATA_DIR_PATH.c_str(), DATA_DIR_PATH.size()) ==
      0) { // is a DataPC file
    ...
    if (MAP.find(base_filepath) !=
      ...
      Logging::Log("[FileSystem::BinkW32::BinkOpen] Loading Bink file: %s from "
                   "mod: %s...\n",
...
DWORD __stdcall BASS_StreamCreateFileHook(BOOL mem, char *file, DWORD offset,
  ...
  if (!mem && _strnicmp(file, DATA_DIR_PATH.c_str(), DATA_DIR_PATH.size()) ==
                  0) { // is a DataPC file
...
DWORD __stdcall BASS_SampleLoadHook(BOOL mem, char *file, DWORD offset,
  ...
  if (!mem && _strnicmp(file, DATA_DIR_PATH.c_str(), DATA_DIR_PATH.size()) ==
    ...
    if (MAP.find(base_filepath) !=
      ...
      Logging::Log("[FileSystem::BASS::SampleLoad] Loading stream file: %s "
                   "from mod: %s...\n",

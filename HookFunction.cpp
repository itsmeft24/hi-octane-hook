...
HookedFunctionInfo HookFunction(unsigned long src, void* dst, const size_t code_size, FunctionHookType type) {
	...
	if (code_size < 5 && (type == FunctionHookType::InlineReplacement || type == FunctionHookType::InlineReplacementJMP)) return info; // You cannot inline hook at code segments smaller than 5 bytes.
		...
		*(void**)src = dst;
		...
		WriteCALL(AsVoidPtr(src), dst); // Write a function call from the source to the detour function.
	}
	else if (type == FunctionHookType::InlineReplacementJMP) {
		SetExecuteReadWritePermission((void*)src, code_size);
		...
		WriteJMP(AsVoidPtr(src), dst); // Write a function call from the source to the detour function.
	}
	return info;
}
